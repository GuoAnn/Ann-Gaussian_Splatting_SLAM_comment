'''
Camera类封装了相机的内参、外参、图像数据及相关操作，主要功能包括：
初始化相机，管理相机位姿（旋转R和平移T）（SLAM优化后会更新位姿）
处理投影矩阵和坐标系变换
计算图像梯度掩码（用于边缘检测）
支持从数据集或GUI初始化相机
提供相机中心、视图变换等关键属性
...
技术亮点
灵活的初始化方式：支持从数据集或GUI交互初始化。
梯度掩码自适应：针对不同数据集（如replica）采用分块或全局阈值。
可微分参数：通过nn.Parameter定义位姿和曝光量的可优化参数，便于与PyTorch优化器集成。
高效的矩阵运算：使用torch的矩阵操作（如bmm）加速投影变换。
'''
import torch
